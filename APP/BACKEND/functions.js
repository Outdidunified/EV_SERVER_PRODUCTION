...
async function handleChargingSession(charger_id, connectorId,startTime, stopTime, Unitconsumed, Totalprice, user, SessionID, connectorTypeValue, chargerErrorCode) {
    const db = await connectToDatabase();
...
async function updateSessionPriceToUser(user, price) {
    try {
        ...
        if (userDocument) {
            let updatedWalletBalance = (userDocument.wallet_bal - sessionPrice).toFixed(2);
            
            // Ensure balance doesn't go negative
            updatedWalletBalance = Math.max(parseFloat(updatedWalletBalance), 0).toFixed(2);
...
async function getAutostop(user){
    try{
        const db = await database.connectToDatabase();
        const UserDetails = await db.collection('users').findOne({ username: user });
...
async function StopBasedOnWallet(firstMeterValues, lastMeterValues, autostopSettings, uniqueIdentifier, connectorId) {
    try {
        const startEnergy = firstMeterValues || 0;
        const lastEnergy = lastMeterValues || 0;
        ...
        //const calculatedUnit = parseFloat(result).toFixed(3);
        const unit = isNaN(calculatedUnit) ? 0 : calculatedUnit;
        console.log(`Total unit consummed: ${lastEnergy}(last energy) - ${startEnergy}(start energy) = ${result}`);
        ...
        // Check if the auto stop conditions are met
        if (autostopSettings.wallet_balance) {
            if ((autostopSettings.wallet_balance - 15) <= formattedSessionPrice) {
                console.log(`Charger ${uniqueIdentifier} stop initiated - due to low wallet balance !`);
                const result = await Chargercontrollers.chargerStopCall(uniqueIdentifier, connectorId);
                if (result === true) {
                    console.log(`AutoStop price: Charger Stopped!`);
                } else {
        ...
        console.log('Failed to calculate price - stop based on wallet:', error);
    }
}
