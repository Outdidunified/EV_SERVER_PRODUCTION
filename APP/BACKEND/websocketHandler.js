...
const {
    ...
    getConnectorId, StopBasedOnWallet
} = require('./functions');
            ...
            ws.on('message', async (message) => {
                ...
                if (Array.isArray(requestData) && requestData.length >= 4) {
                    ...
                    if (requestName === "BootNotification") {
                        ...
                        if (errors.length === 0) {
                            ...
                            if(vendorErrorCode === undefined){
                                ...
                                chargerErrorCode = vendorErrorCode;
                            }
                        ...
                        if (requestErrors.length === 0) {
                            ...
                            const processMeterValues = async (firstMeter, lastMeter, settings, identifier, connector) => {
                                if (settings.isUnitChecked) {
                                    console.log('unit autostop - new session initiated');
                                    await autostop_unit(firstMeter, lastMeter, settings, identifier, connector);
                                } 
                                if (settings.isPriceChecked) {
                                    console.log('price autostop - new session initiated');
                                    await autostop_price(firstMeter, lastMeter, settings, identifier, connector);
                                }

                                await StopBasedOnWallet(firstMeter, lastMeter, settings, identifier, connector);
                            };
                    ...
                    if (sessionFlags.get(key) === 1) {
                        ...
                        if(user){
                            await handleChargingSession(uniqueIdentifier, connectorId, startTime, stopTime, unit, sessionPrice, user, chargingSessionID.get(key), connectorTypeValue, chargerErrorCode);
                        }else{
