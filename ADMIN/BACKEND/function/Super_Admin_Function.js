...
async function UpdateCharger(req, res) {
    try {
        const { charger_id, charger_model, charger_type, vendor, max_current, max_power, wifi_module, bluetooth_module, modified_by, connectors } = req.body;

        // Validate the input - ensure charger_id and other required fields are provided
        if (!charger_id || !vendor || !max_current || !charger_model || !charger_type || !max_power || !wifi_module || !bluetooth_module || !modified_by) {
            return res.status(400).json({ message: 'Charger ID, charger_model, charger_type, Vendor, Max Current, Max Power, and Modified By are required' });
        }
